...
# ----- Helper for bare keys (simple strings) -----
bare_key_strat = st.text(
    min_size=1,
    max_size=8,
    alphabet=st.characters(
        whitelist_categories=('Lu', 'Ll', 'Nd'),
...
def document_strategy(draw):
    ...
    for idx in range(num_stmts):
        if idx == deep_pos:
            ...
            key_str = draw(bare_key_strat.filter(lambda s: s not in used_key_reprs))
            used_key_reprs.add(key_str)
            statements.append(KeyValue(key_str, deep_val))

        elif idx == bool_pos:
            bool_val = draw(valid_bool)
            key_str = draw(bare_key_strat.filter(lambda s: s not in used_key_reprs))
            used_key_reprs.add(key_str)
            statements.append(KeyValue(key_str, bool_val))
            ...
            if stmt_type == 'kv':
                ...
                if key_kind == 'simple':
                    ...
                    content = draw(bare_key_strat)
                    key = QuotedKey(content, is_lit)
                    used_key_reprs.add(f'quoted:{content}:{is_lit}')

                else:  # dotted
                    parts_len = draw(st.integers(min_value=2, max_value=4))
                    parts = []
                    for _ in range(parts_len):
                        part = draw(bare_key_strat)
                        parts.append(part)
                    dotted_repr = '.'.join(parts)
                    # ensure uniqueness of the dotted representation
                    while dotted_repr in used_key_reprs:
                        parts = [draw(bare_key_strat) for _ in range(parts_len)]
                        dotted_repr = '.'.join(parts)
                    used_key_reprs.add(dotted_repr)
                    key = DottedKey(parts_len, parts)
