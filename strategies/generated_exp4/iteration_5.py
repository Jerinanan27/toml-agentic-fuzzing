...
def document_strategy(draw):
    num_stmts = draw(st.integers(min_value=8, max_value=15))

    # Choose positions for deep value and boolean value
    ...
    for idx in range(num_stmts):
        if idx == deep_pos:
            ...
            key = draw(bare_key_strat.filter(lambda s: s not in used_keys))
            used_keys.add(key)
